...
def visualize_packing1(df):
    ...
    for id_carton in list_carton:
        ...
        result = visualize_packing_animated(df_temp, id_carton)
        results.append(result)
    return results

def pack_articles_animated(articles, carton_length, carton_width, carton_height):
...
def visualize_packing_animated(df, id_carton):
    ...
    for orientation in carton_orientations:
        packing_steps, volume_utilisé = pack_articles_animated(articles, *orientation)
        if len(packing_steps[-1]) > len(best_packing_steps[-1] if best_packing_steps else []) or (len(packing_steps[-1]) == len(best_packing_steps[-1] if best_packing_steps else []) and volume_utilisé > best_volume_utilisé):
            best_orientation = orientation
            best_volume_utilisé = volume_utilisé
            best_packing_steps = packing_steps
    
    carton_length, carton_width, carton_height = best_orientation
    
    # Create a directory to store the frame images
    os.makedirs('temp_frames', exist_ok=True)
    
    frame_files = []
    for step, placed_articles in enumerate(best_packing_steps):
        fig = go.Figure()
        fig.add_trace(draw_parallelepiped(0, 0, 0, carton_length, carton_width, carton_height, 'rgba(200, 200, 200, 0.1)'))

        for article in placed_articles:
        ...
        fig.update_layout(
            ...
            title=f'Visualisation de l\'emballage dans le carton {id_carton} (Étape {step+1}/{len(best_packing_steps)})',
        ...
        frame_file = f'temp_frames/frame_{step:03d}.png'
        fig.write_image(frame_file)
        frame_files.append(frame_file)
    
    # Create the GIF
    images = [imageio.imread(f) for f in frame_files]
    gif_file = f"app/static/images/images_emballage/viz_carton{id_carton}_animated.gif"
    imageio.mimsave(gif_file, images, duration=500)  # 500ms per frame
    ...
    return {
        ...
        'articles_placés': len(best_packing_steps[-1]),
...
# visualize_packing_animated(df, id_carton=1)
